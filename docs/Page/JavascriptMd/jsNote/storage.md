#  sessionStorage 和 localStorage 
 ...
 `localStorage`保存的数据长期存在，下一次访问该网站的时候，网页可以直接读取以前保存的数据。 IE8以上的IE版本才支持localStorage这个属性 。 localStorage在浏览器的隐私模式下面是不可读取的并且 不能被爬虫抓取到 

 `localStorage`和`sessionStorage`在不同的浏览器存储的上限也不一样，但大多数浏览器把上限限制在5MB左右。 
